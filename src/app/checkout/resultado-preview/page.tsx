/**
 * PÁGINA BORRADOR — /checkout/resultado-preview
 * Solo para diseño. NO tocar /checkout/resultado hasta aprobar este diseño.
 * Datos completamente falsos para previsualizar todos los estados.
 * Cambiar ?estado=APPROVED | REJECTED | FAILED | PENDING en la URL
 */
...
const FAKE_ITEMS = [
  { id: 1, name: "KIT DE SUSPENSION YAMAHA YBR 125 2018-2022", quantity: 2, total: "180000", image: null },
  { id: 2, name: "FILTRO DE ACEITE HONDA CB 190 2019", quantity: 1, total: "35000", image: null },
];

const FAKE_ORDER = {
  reference: "4872",
  fecha: "30 de marzo de 2026, 10:45 a. m.",
  subtotal: 215000,
  envio: 0,
  total: "$215.000 COP",
...
export default async function ResultadoPreviewPage({ searchParams }: PreviewProps) {
  const { estado = "APPROVED" } = await searchParams;
  ...
  const estadoConfig: Record<string, {
    ...
    REJECTED: {
      ...
      mensaje: "La entidad financiera ha declinado la transacción. Verifica los datos de tu tarjeta e intenta de nuevo.",
      barColor: "bg-red-500",
    },
    FAILED: {
      ...
      mensaje: "Hubo un inconveniente técnico al procesar tu pago. Por favor intenta de nuevo más tarde.",
  ...
  const cfg = estadoConfig[estado] || estadoConfig["APPROVED"];
  const isSuccess = estado === "APPROVED" || estado === "PENDING";
  const isError = estado === "REJECTED" || estado === "FAILED";
    ...
    <div className="flex flex-col min-h-screen bg-[#F8F7F5]">
      ...
      <main className="flex-grow px-5 py-12">
        <div className="max-w-7xl mx-auto">
          ...
          {/* ── BLOQUE SUPERIOR: estado + mensaje ── */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* ── COLUMNA IZQUIERDA: detalles del pedido (2/3) ── */}
              ...
              <div className="bg-white border border-gray-100">
                ...
                <div className="divide-y divide-gray-50">
                  {FAKE_ITEMS.map((item) => (
                    <div key={item.id} className="flex items-center gap-4 px-6 py-4">
                      {/* Imagen placeholder */}
                      <div className="w-14 h-14 bg-gray-100 shrink-0 flex items-center justify-center">
                ...
                <div className="px-6 py-5 border-t border-gray-100 bg-[#FAFAFA] space-y-3">
                  <div className="flex justify-between text-[12px]">
                    ...
                    <span className="font-bold text-secondary">${FAKE_ORDER.subtotal.toLocaleString("es-CO")}</span>
                  </div>
                  <div className="flex justify-between text-[12px]">
                    <span className="text-gray-500 font-bold uppercase tracking-widest">Envío a {FAKE_ORDER.ciudad}</span>
                    <span className="font-bold text-green-600">Gratis</span>
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                    <span className="text-[11px] font-black uppercase tracking-widest text-gray-400">Total pagado</span>
                    <span className="text-xl font-black text-primary">{FAKE_ORDER.total}</span>
                  </div>
                </div>
              </div>

              {/* Seguimiento — solo si fue aprobado */}
              {isSuccess && (
                <div className="bg-white border border-gray-100">
                  <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                    <span className="w-1 h-5 bg-primary inline-block"></span>
            ...
            {/* ── COLUMNA DERECHA: info del pedido (1/3) ── */}
            <div className="flex flex-col gap-6">
              ...
              <div className="bg-white border border-gray-100">
                ...
                <div className="divide-y divide-gray-50">
                  {[
                    { label: "Nº de pedido", value: `#${FAKE_ORDER.reference}` },
                    { label: "Fecha", value: FAKE_ORDER.fecha },
                    { label: "Cliente", value: FAKE_ORDER.cliente },
                    { label: "Ciudad de envío", value: FAKE_ORDER.ciudad },
                  ].map((row) => (
              ...
              <Link
                href="/mis-pedidos"
                className="flex items-center justify-between bg-secondary text-white px-6 py-6 hover:bg-black transition-colors group"
              >
                <div>
